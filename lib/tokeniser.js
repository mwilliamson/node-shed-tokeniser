...
var Tokeniser = exports.Tokeniser = function(options) {
    ...
    var tokenise = function(source) {
        ...
        while (!input.isAtEnd()) {
            ...
            } else if (input.peekString(-currentIndentation.length) === currentIndentation && input.peek() === " ") {
                var indent = readIndent(input);
                result.push(tokens.indent(indent, createTokenSource()));
                indents.push((indents[indents.length - 1] || "") + indent);
            } else if (isWhitespace(input)) {
                result.push(tokens.whitespace(readWhitespace(input, currentIndentation), createTokenSource()));
...
var readWhitespace = function(input, currentIndentation) {
    return input.next() + input.takeWhile(function(input) {
        return isWhitespace(input) && input.peekString(-currentIndentation.length) !== currentIndentation;
    });
};
