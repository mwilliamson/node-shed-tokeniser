...
var Tokeniser = exports.Tokeniser = function(options) {
    ...
    var tokenise = function(source) {
        ...
        var endSource = input.rangeTo(input);

        if (indents.length > 0) {
            result.push(tokens.newLine(endSource));
            indents.forEach(function() {
                result.push(tokens.dedent(endSource));
            });
        }

        result.push(tokens.end(endSource));
        var errors = [];
        return {
            tokens: result,
            errors: errors
